package ai.gollek.peutui.core.buffer;

import ai.gollek.peutui.core.layout.Size;
import ai.gollek.peutui.core.text.AnsiCodes;
import ai.gollek.peutui.core.text.GraphemeWidth;
...
public final class ScreenBuffer {
    ...
    /** Writes {@code text} (which may contain ANSI SGR codes) starting at {@code (x, y)}, clipped to bounds. */
    ...
    /** Renders one row to a single ANSI-styled string, coalescing consecutive cells with identical styling. */
    ...
    /** A single row that changed between two frames, with row index and the fully rendered replacement content. */
