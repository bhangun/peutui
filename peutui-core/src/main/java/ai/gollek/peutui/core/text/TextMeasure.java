...
public final class TextMeasure {
    ...
    /** Result of slicing a line: the resulting (possibly re-styled) text and its true visible width. */
    ...
    /** Extracts the visible columns {@code [startCol, startCol+length)} from {@code line}. */
    ...
    /** Like {@link #sliceByColumn} but also returns the actual visible width extracted. */
     ...
     */
    public static Segments extractSegments(String line, int beforeEnd, int afterStart, int afterLen, boolean strictAfter) {
    ...
    /** Truncates {@code text} to at most {@code maxWidth} visible columns, appending {@code ellipsis} if cut. */
    public static String truncateToWidth(String text, int maxWidth, String ellipsis, boolean pad) {
        ...
        // Re-slice with styling preserved using the width-aware slicer once we know the cut point.
        int visibleSoFar = 0;
    ...
    private static String withStyling(String original, String plainFallback) {
        // If the original had no ANSI codes, plainFallback is equivalent and cheaper to return.
