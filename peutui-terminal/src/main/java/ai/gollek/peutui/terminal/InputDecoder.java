package ai.gollek.peutui.terminal;
...
import ai.gollek.peutui.core.event.PasteEvent;
 ...
 *
 * <p>Feed characters one at a time via {@link #feed(int)}; fully decoded
...
public final class InputDecoder {
    ...
    /** Feeds one decoded character (codepoint) into the decoder; returns zero or more resolved events. */
    public List<InputEvent> feed(int codepoint) {
        ...
        // Not an escape sequence at all: flush whatever was pending as-is, then handle this char.
        pending.setLength(0);
    ...
    private static boolean isPrefixOfKnownSequence(String buf) {
        ...
        return PASTE_START.startsWith(buf) || buf.startsWith("\u001b[") || buf.startsWith("\u001bO") || buf.equals("\u001b");
    }
    ...
    /** True while a bracketed-paste block is being accumulated and hasn't resolved to a {@link PasteEvent} yet. */
    public boolean isInPaste() {
        return inPaste;
    }

    /** If a bare ESC has been buffered with nothing following, flush it out as the Escape key. */
