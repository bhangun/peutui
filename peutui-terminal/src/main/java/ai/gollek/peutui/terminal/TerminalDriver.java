...
public interface TerminalDriver extends AutoCloseable {
    ...
    /** Puts the terminal into raw mode (no line buffering/echo) and enables the alternate screen. */
    void start();
    ...
    /** Registers a listener invoked for every decoded input event (key press, resize, paste). */
    void onInput(Consumer<InputEvent> listener);

    /** Writes a raw pre-rendered frame (ANSI included) to the terminal, without an implicit trailing newline. */
