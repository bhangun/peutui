...
import ai.gollek.peutui.provider.FailoverProviderSelectionStrategy;
import ai.gollek.peutui.provider.ModelProvider;
import ai.gollek.peutui.provider.ProviderRegistry;
import ai.gollek.peutui.provider.ProviderSelectionStrategy;
import ai.gollek.peutui.provider.RoundRobinProviderSelectionStrategy;
import ai.gollek.peutui.provider.SingleProviderSelectionStrategy;
        ...
        return switch (config.provider().selection()) {
            case SINGLE -> new SingleProviderSelectionStrategy(
                    config.provider().defaultProviderId()
                            .orElseThrow(() -> new IllegalStateException("peutui.provider.default-provider-id is required when selection=single")));
            ...
            case FAILOVER -> new FailoverProviderSelectionStrategy(
                    config.provider().failoverOrder()
                            .orElseThrow(() -> new IllegalStateException("peutui.provider.failover-order is required when selection=failover")));
        };
    }
}
