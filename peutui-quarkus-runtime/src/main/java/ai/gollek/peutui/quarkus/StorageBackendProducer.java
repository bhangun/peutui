...
import ai.gollek.peutui.storage.CloudStorageBackend;
import ai.gollek.peutui.storage.LocalDatabaseStorageBackend;
import ai.gollek.peutui.storage.LocalFileStorageBackend;
...
public final class StorageBackendProducer {
    ...
    private StorageBackend build(PeutuiConfig.StorageConfig storageConfig, String label) {
        return switch (storageConfig.mode()) {
            case LOCAL_FILE -> new LocalFileStorageBackend(Path.of(storageConfig.localFile().rootPath()).resolve(label));
            case LOCAL_DATABASE -> {
            ...
            case CLOUD -> {
                String baseUrl = storageConfig.cloud().baseUrl()
                        .orElseThrow(() -> new IllegalStateException("peutui.storage.cloud.base-url is required when storage mode is 'cloud'"));
