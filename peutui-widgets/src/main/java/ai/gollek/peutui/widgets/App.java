package ai.gollek.peutui.widgets;

import ai.gollek.peutui.core.buffer.ScreenBuffer;
import ai.gollek.peutui.core.component.Component;
import ai.gollek.peutui.core.component.RenderContext;
import ai.gollek.peutui.core.event.InputEvent;
import ai.gollek.peutui.core.event.KeyCode;
import ai.gollek.peutui.core.event.KeyEvent;
import ai.gollek.peutui.core.event.ResizeEvent;
import ai.gollek.peutui.core.layout.BoxConstraints;
import ai.gollek.peutui.core.layout.Rect;
 ...
 *
 * <p>Not itself a strategy point - App is deliberately the "glue" layer that
...
public final class App {
    ...
    /** Installs a handler invoked before per-component dispatch; returning true consumes the event (e.g. global Ctrl+C quit). */
    ...
    /** Blocks the calling thread until {@link #stop()} is invoked (e.g. via Ctrl+C on the input thread). */
    ...
    private void onInput(InputEvent event) {
        try {
            ...
            if (event instanceof KeyEvent key && key.code() == KeyCode.CHARACTER && key.ctrl() && "c".equals(key.character())) {
                stop();
                return;
            }
    ...
    /** Triggers an immediate repaint - call after external state changes too (e.g. a streamed agent token arriving). */
