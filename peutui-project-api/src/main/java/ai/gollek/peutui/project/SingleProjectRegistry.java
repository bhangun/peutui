package ai.gollek.peutui.project;

import java.util.List;
import java.util.Optional;

/** {@link ProjectRegistry} strategy that pins the application to exactly one fixed project for its whole lifetime. */
public final class SingleProjectRegistry implements ProjectRegistry {
    ...
    public void setActive(String projectId) {
        if (!project.id().equals(projectId)) {
            throw new UnsupportedOperationException("This application is configured for a single fixed project: " + project.id());
        }
    }
}
