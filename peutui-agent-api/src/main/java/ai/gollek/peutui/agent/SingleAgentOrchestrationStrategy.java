    ...
    private Agent resolveAgent() {
        ...
        if (defaultAgentId != null) {
            return registry.find(defaultAgentId)
                    .orElseThrow(() -> new IllegalStateException("Default agent '" + defaultAgentId + "' is not registered"));
        }
        if (all.size() > 1) {
            throw new IllegalStateException(
                    "SingleAgentOrchestrationStrategy requires exactly one registered agent, or an explicit defaultAgentId; found " + all.size());
        }
        return all.get(0);
    }
}
