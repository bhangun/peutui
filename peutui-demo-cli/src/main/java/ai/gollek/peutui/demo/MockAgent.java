...
import ai.gollek.peutui.agent.AgentDescriptor;
import ai.gollek.peutui.agent.AgentEvent;
import ai.gollek.peutui.agent.AgentMessage;
import ai.gollek.peutui.provider.ProviderEvent;
import ai.gollek.peutui.provider.ProviderMessage;
import ai.gollek.peutui.provider.ProviderRegistry;
import ai.gollek.peutui.provider.ProviderRequest;
...
public final class MockAgent implements Agent {
    ...
    public AgentDescriptor descriptor() {
        return new AgentDescriptor("assistant", "Assistant", "General-purpose demo agent", List.of(AgentCapability.GENERAL));
    }
            ...
            public void onNext(ProviderEvent item) {
                switch (item) {
                    ...
                    case ProviderEvent.ToolUse toolUse ->
                            out.submit(new AgentEvent.ToolCallStarted(id(), toolUse.toolName(), toolUse.toolCallId(), toolUse.argumentsJson()));
                    case ProviderEvent.Done done ->
                            out.submit(new AgentEvent.TurnCompleted(id(), AgentMessage.assistant(id(), accumulated.toString())));
